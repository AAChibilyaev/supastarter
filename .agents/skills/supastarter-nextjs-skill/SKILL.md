...
3. **Read the source-of-truth files** for this task type before writing anything:
    ...
    - i18n → `packages/i18n/translations/{en,de,es,fr}/<scope>.json`
...
ls packages/api/modules/   # current: admin, ai, billing-wallet, notifications, organizations, payments, search, users
...
- **Current focus (v0.5)**: public marketing site at `apps/marketing` (target host `aacsearch.com`).
- **Already shipped (v0.x)**: single-doc upsert, multi-search, browser SDK (`@repo/search-client`), scoped tokens, per-key origin allow-list + rate-limit, per-org plan quota, zero-downtime alias-swap reindex, DB-first ingest pipeline.
- **Deferred — do NOT implement unless explicitly asked**: Stripe billing wired to search-units (**v0.6**), public docs site `apps/docs` (**v0.7**), self-host quickstart + Helm chart (**v1.0**).
- **Half-orphaned but present in code**: AI Wallet (`@repo/billing-wallet`, `@repo/ai-core`, kopecks ledger, Tochka top-up driver, oRPC `billing-wallet` module, settings UI). Code is intact but **not** wired to search-units billing yet (that's v0.6). Do not extend without an explicit task — and confirm scope before doing so.
- **Removed in last commit (`84481e3`) — do NOT recreate**: `apps/saas/app/api/cron/sync-subscriptions/route.ts`, `packages/api/lib/wallet-sync.ts`.

When a request comes in, **first** classify it: `v0.x search/keys/tokens/quota/usage` ✅ implement | `v0.5 marketing copy/page` ✅ implement | `v0.6/v0.7/v1.0` → **deferred, ask before coding** | `wallet/ai/dslmrank` → **out of scope, ask before coding**. State the classification in your first reply.
...
11. **i18n: every user-visible string lands in ALL 4 locales (`en`, `de`, `es`, `fr`) in the same change.** Adding a key only to `en` is a **production bug** — `next-intl` falls back silently and the other 3 locales render the key path or empty. Files: `packages/i18n/translations/{en,de,es,fr}/{saas,marketing,mail,shared}.json`. Pick the right scope file (saas-only feature → `saas.json`, marketing page → `marketing.json`, etc.). Verify with `rg '"<your.key>"' packages/i18n/translations/` — must show 4 hits.
12. **REUSE FIRST — never reinvent.** Before creating ANY new component / hook / procedure / query / helper / type / lib function: run the relevant grep from the MANDATORY pre-write protocol above and inspect results. Hard ranking of where to look:
    ...
    - **Workspace package**: 16 already exist. Adding a 17th requires user confirmation + ≥2 internal consumers OR a customer-facing SDK use case.
...
| `packages/billing-wallet` or `packages/ai-core`                                                | **confirm with user first** — this domain is half-orphaned (not in v0.x)                                                                                                                                           |
| Anything in `apps/marketing`                                                                   | this is **v0.5 current focus** — proceed; check `packages/i18n/translations/*/marketing.json` for keys                                                                                                             |
| Adding / changing a user-visible string anywhere                                               | **all 4 locale files** (`en`, `de`, `es`, `fr`) in the matching scope (`saas`/`marketing`/`mail`/`shared`) updated in the same change; verify `rg '"<your.key>"' packages/i18n/translations/` returns 4 hits       |
...
| Stripe billing wired to search-units, search-unit metering                                     | **deferred to v0.6** — confirm before building                                                                                                                                                                     |
| `agents.md`, `docs/plans/aacsearch-prd.md`, or any `**/skills/**/SKILL.md`                     | mirror SKILL.md trees via the Autonomous sync rule below; verify with `diff -qr`                                                                                                                                   |
| Adding a new workspace package (`packages/<new>/`)                                             | confirm with user — current count is **16**; new packages need a real reuse case (≥2 internal consumers, OR customer-facing SDK with a stated external consumer)                                                   |
...
### R2. Add a marketing page (v0.5 current focus)

- **Files**: `apps/marketing/app/[locale]/<route>/page.tsx`, `apps/marketing/modules/<area>/components/<section>.tsx`, `packages/i18n/translations/{en,de,es,fr}/marketing.json`, optionally `apps/marketing/sitemap.ts`
- **Preconditions**: copy approved by user; route belongs in marketing app, **not** saas
- **Steps**: 1) add translation keys in **all 4 locales** (skipping any → silent fallback bug); 2) build a Server Component composing existing blocks from `apps/marketing/modules/shared/components` first — only create new sections if no existing block fits; 3) wire the route via App Router; 4) update sitemap if public
...
packages/                 # 16 workspace packages — all imported via @repo/<name>
  api/                    # Hono + oRPC. modules/{admin,ai,billing-wallet,notifications,organizations,payments,search,users}, orpc/{router,procedures,handler,middleware}
  auth/                   # Better Auth (auth.ts, client.ts, lib/, plugins/)
  database/               # BOTH Prisma (prisma/) AND Drizzle (drizzle/) coexist; 25 Prisma models; query helpers in prisma/queries/
  ...
  billing-wallet/         # AI Wallet domain: kopecks ledger, reserve→commit/release, Tochka top-up driver, scoped tokens
  search/                 # Typesense client, versioned collections + alias swap, ingest buffer worker, public-handler bulk/multi-search
  ...
  payments/               # Stripe, Lemonsqueezy, Polar, Creem, DodoPayments — provider/; per-plan limits in config.ts (incl. searchLimits)
...
- [ ] All 4 locale files updated if any user-visible string was added or changed (Invariant #11; verify `rg '"<key>"' packages/i18n/translations/` returns 4 hits)
