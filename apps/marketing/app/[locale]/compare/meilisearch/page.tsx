...
import { CompareMeilisearchGrid } from "../../../../modules/compare/components/CompareMeilisearchGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "compareMeilisearchPage" });
	return {
		title: t("title"),
...
export default async function CompareMeilisearchPage(props: {
	params: Promise<{ locale: string }>;
}) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: tb("vs.meilisearch"), url: `${baseUrl}/${locale}/compare/meilisearch` },
	];

	return (
		<>
			<BreadcrumbSchema items={breadcrumbs} baseUrl={baseUrl} />
			<FAQPageSchema items={faqItems} id={`/${locale}/compare/meilisearch#faq`} />
