...
import { CurationsGrid } from "../../../../modules/features/components/CurationsGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "featuresCurationsPage" });
	return {
		title: t("title"),
...
export default async function FeaturesCurationsPage(props: {
	params: Promise<{ locale: string }>;
}) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: "Curations", url: `${baseUrl}/${locale}/features/curations` },
	];
