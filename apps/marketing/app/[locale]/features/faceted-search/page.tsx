...
import { FacetedSearchGrid } from "../../../../modules/features/components/FacetedSearchGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "featuresFacetedSearchPage" });
	return {
		title: t("title"),
...
export default async function FeaturesFacetedSearchPage(props: {
	params: Promise<{ locale: string }>;
}) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: "Faceted Search", url: `${baseUrl}/${locale}/features/faceted-search` },
	];
