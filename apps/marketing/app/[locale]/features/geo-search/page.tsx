...
import { GeoSearchGrid } from "../../../../modules/features/components/GeoSearchGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "featuresGeoSearchPage" });
	return {
		title: t("title"),
...
export default async function FeaturesGeoSearchPage(props: {
	params: Promise<{ locale: string }>;
}) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: "Geo Search", url: `${baseUrl}/${locale}/features/geo-search` },
	];
