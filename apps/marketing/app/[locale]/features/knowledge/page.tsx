...
import { KnowledgeGrid } from "../../../../modules/features/components/KnowledgeGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "featuresKnowledgePage" });
	return {
		title: t("title"),
...
export default async function FeaturesKnowledgePage(props: {
	params: Promise<{ locale: string }>;
}) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: "Knowledge", url: `${baseUrl}/${locale}/features/knowledge` },
	];
