...
import { ScopedTokensGrid } from "../../../../modules/features/components/ScopedTokensGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "featuresScopedTokensPage" });
	return {
		title: t("title"),
...
export default async function FeaturesScopedTokensPage(props: {
	params: Promise<{ locale: string }>;
}) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: "Scoped Tokens", url: `${baseUrl}/${locale}/features/scoped-tokens` },
	];
