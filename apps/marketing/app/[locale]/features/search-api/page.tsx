...
import { SearchApiGrid } from "../../../../modules/features/components/SearchApiGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "featuresSearchApiPage" });
	return {
		title: t("title"),
...
export default async function FeaturesSearchApiPage(props: {
	params: Promise<{ locale: string }>;
}) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: "Search API", url: `${baseUrl}/${locale}/features/search-api` },
	];
