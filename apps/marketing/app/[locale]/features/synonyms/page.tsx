...
import { SynonymsGrid } from "../../../../modules/features/components/SynonymsGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "featuresSynonymsPage" });
	return {
		title: t("title"),
...
export default async function FeaturesSynonymsPage(props: { params: Promise<{ locale: string }> }) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: "Synonyms", url: `${baseUrl}/${locale}/features/synonyms` },
	];
