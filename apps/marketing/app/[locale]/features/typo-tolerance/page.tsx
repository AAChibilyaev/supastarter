...
import { TypoToleranceGrid } from "../../../../modules/features/components/TypoToleranceGrid";

export async function generateMetadata(props: {
	params: Promise<{ locale: string }>;
}): Promise<Metadata> {
	const { locale } = await props.params;
	const t = await getTranslations({ locale, namespace: "featuresTypoTolerancePage" });
	return {
		title: t("title"),
...
export default async function FeaturesTypoTolerancePage(props: {
	params: Promise<{ locale: string }>;
}) {
	const { locale } = await props.params;
	...
	const breadcrumbs = [
		...
		{ name: "Typo Tolerance", url: `${baseUrl}/${locale}/features/typo-tolerance` },
	];
