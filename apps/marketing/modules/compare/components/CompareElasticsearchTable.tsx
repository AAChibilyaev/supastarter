...
export function CompareElasticsearchTable() {
						...
						{FEATURE_KEYS.map((key) => (
							<div key={key} className="p-4 rounded-lg border border-border/60">
								...
								<div className="gap-2 text-sm grid grid-cols-2">
									...
									<div>
										<div className="text-xs mb-1 text-muted-foreground">Elasticsearch</div>
										<div className="rounded px-2 py-1">
											{t(`table.features.${key}.elasticsearch`)}
										</div>
									</div>
								</div>
							</div>
							...
							<TableBody>
								{FEATURE_KEYS.map((key) => (
									<TableRow key={key}>
										...
										<TableCell>
											<FeatureCell text={t(`table.features.${key}.elasticsearch`)} />
										</TableCell>
									</TableRow>
								))}
							</TableBody>
