...
export function ElasticsearchDecisionMatrix() {
	const t = useTranslations("compareElasticsearch");

	const aacItems = [0, 1, 2, 3, 4];
	const elasticsearchItems = [0, 1, 2, 3, 4];
					...
					<div className="md:grid-cols-2 gap-8 grid">
						...
						<div className="p-6 md:p-8 rounded-xl border border-border/60">
							<h3 className="font-semibold text-xl mb-4">
								{t("decisionMatrix.whenElasticsearch.heading")}
							</h3>
							<ul className="space-y-3">
								{elasticsearchItems.map((i) => (
									<li key={i} className="gap-3 text-sm flex">
										...
										<span className="leading-relaxed text-muted-foreground">
											{t(`decisionMatrix.whenElasticsearch.items.${i}` as Parameters<typeof t>[0])}
										</span>
									</li>
								))}
							</ul>
