...
const DEMOS: Record<UseCaseId, UseCaseConfig> = {
		...
		docs: [
			{
				...
				subtitle: "$129 · In stock · Free delivery",
				badge: "Best match",
				tags: ["nike", "running", "shoes"],
			},
			{
				id: "2",
				title: "Nike React Infinity — Size 42",
				subtitle: "$149 · In stock · 2-day shipping",
				badge: undefined,
				tags: ["nike", "stability", "shoes"],
			},
			{
				id: "3",
				title: "Nike ZoomX Vaporfly",
				subtitle: "$250 · Limited stock · Size 41–43",
				badge: undefined,
				tags: ["nike", "carbon", "shoes"],
			},
			{
				id: "4",
				title: "Adidas Ultraboost 23 — Size 42",
			...
			{
				id: "5",
				title: "Nike Air Force 1 Low — Size 42",
				subtitle: "$110 · In stock",
				badge: undefined,
				tags: ["nike", "casual", "shoes"],
			},
			{
				id: "6",
				title: "Samsung Galaxy S24 Ultra",
				subtitle: "$1,199 · 12GB RAM · Titanium",
				badge: undefined,
				tags: ["samsung", "phone"],
			},
			{
				...
				subtitle: "$1,099 · Natural Titanium · In stock",
				badge: undefined,
			...
			},
			{
				...
				subtitle: "$349 · Noise-cancelling · Wireless",
				badge: undefined,
	...
	saas: {
		...
		docs: [
			...
			{
				id: "3",
				title: "Project Alpha — Backend migration",
				subtitle: "Project · In progress · Owner: John D.",
				badge: "Project",
			...
			{
				id: "5",
				title: "Invoice template — Monthly SaaS",
				subtitle: "Document · Last edited 2 days ago",
				badge: "Doc",
				tags: ["invoice", "template", "saas"],
			},
			{
				id: "6",
				title: "Auth settings — SSO configuration",
				subtitle: "Settings · Security · Updated 4 days ago",
				badge: "Settings",
				tags: ["settings", "auth", "sso"],
			},
		],
	...
	docs: {
		...
		docs: [
			{
				...
				subtitle: "API Reference · Quotas & Limits · 3 min read",
				badge: "API Ref",
				tags: ["rate", "limit", "429", "quota"],
			},
			{
				id: "2",
				title: "API Keys & Scoped Tokens — Security Guide",
				subtitle: "Guides · Security · 5 min read",
				badge: "Guide",
				tags: ["api", "key", "scoped", "token", "auth"],
			},
			{
				id: "3",
				title: "Error Reference — 4xx Responses",
				subtitle: "API Reference · 401, 403, 429, 422",
				badge: "API Ref",
				tags: ["error", "401", "403", "429"],
			},
			{
				id: "4",
				title: "Quick Start — First Search in 5 Minutes",
			...
			{
				id: "5",
				title: "JavaScript SDK — v3.1.0 Reference",
				subtitle: "SDK · npm install @aacsearch/sdk",
				badge: "SDK",
				tags: ["sdk", "javascript", "npm"],
			},
			{
				id: "6",
				title: "Authentication overview — Bearer tokens",
				subtitle: "Guides · Auth · 4 min read",
				badge: "Guide",
				tags: ["auth", "bearer", "token", "401"],
			},
		],
		typos: {
			"scoped toke": "scoped token",
		...
		aiAnswer:
			"Rate limits are enforced per API key. Default is 100 req/s on Pro and 1,000 req/s on Enterprise. When exceeded, the API returns HTTP 429 with a Retry-After header. Use exponential backoff for automatic retry.",
		scoreReasons: [
	...
	internal: {
		...
		facets: [
			{
				...
				options: [
					...
					{ value: "Deployment", count: 15 },
					{ value: "Config", count: 9 },
				],
			},
			{
				key: "priority",
		...
		docs: [
			{
				id: "1",
				title: "INC-2024-0089 — Auth service 401 spike",
				subtitle: "Incident · High · Resolved 18min · May 2024",
				badge: "Incident",
				tags: ["incident", "auth", "high", "401"],
			},
			{
				id: "2",
				title: "DEPLOY-441 — Auth service v2.3.1 → prod",
				subtitle: "Deployment · Completed · 3 days ago",
				badge: "Deploy",
				tags: ["deploy", "prod", "auth"],
			},
			{
				id: "3",
				title: "AUTH_SECRET_ROTATION — Config",
				subtitle: "Config · Secrets manager · Changed 7d ago",
				badge: "Config",
				tags: ["config", "auth", "secret", "redis"],
			},
			{
				id: "4",
				title: "INC-2024-0072 — Redis timeout cascade",
				subtitle: "Incident · Medium · Post-mortem available",
				badge: "Incident",
				tags: ["incident", "redis", "timeout"],
			},
			{
				id: "5",
				title: "Runbook — Auth service recovery steps",
				subtitle: "Runbook · SRE team · Updated 2024-03",
				badge: "Runbook",
				tags: ["runbook", "auth", "sre"],
			},
			{
				id: "6",
				title: "redis.conf — Production cluster config",
				subtitle: "Config · Infrastructure · maxmemory 32gb",
				badge: "Config",
				tags: ["config", "redis", "prod"],
			},
		],
		typos: {
	...
	return (
		<section className="section-padding border-b border-border bg-muted/10">
			<div className="container">
					...
					<div className="gap-1 p-1 flex flex-wrap items-center rounded-xl border border-border bg-muted">
						{USE_CASE_ORDER.map((id) => (
							<button
								...
								className={cn(
									"px-3 py-1.5 text-sm font-light rounded-lg transition-all duration-150",
									activeCase === id
				...
				{/* Demo shell */}
				<div className="mt-8 max-w-3xl shadow-sm mx-auto overflow-visible rounded-xl border border-border bg-card">
					...
					<div className="relative">
						<div className="gap-3 px-4 py-3 flex items-center border-b border-border bg-muted/20">
							...
							<input
								...
								className="text-sm flex-1 bg-transparent text-foreground outline-none placeholder:text-muted-foreground/50"
							/>
							{query && (
								<button
									type="button"
						...
						{showAC && acSuggestions.length > 0 && (
							<div
								ref={acRef}
								className="left-0 right-0 shadow-lg absolute top-full z-20 overflow-hidden rounded-b-xl border border-t-0 border-border bg-card"
							>
								{acSuggestions.map((s) => (
									<button
										key={s}
										type="button"
										className="gap-3 px-4 py-2.5 text-sm flex w-full items-center text-left transition-colors hover:bg-muted"
					...
					{/* Results area */}
					<div className="sm:flex-row flex min-h-[260px] flex-col">
						{/* Facets */}
						<div className="sm:border-b-0 sm:border-r p-4 sm:w-40 space-y-4 w-full shrink-0 border-b border-border">
							{demo.facets.slice(0, 2).map((facet) => (
								<div key={facet.key}>
									<p className="font-medium tracking-wide mb-2 text-[11px] text-foreground/60 uppercase">
										{facet.label}
									</p>
											...
											>
												<div className="gap-1.5 flex items-center">
													<input
														...
														readOnly
													/>
													<span className="text-xs font-light text-muted-foreground transition-colors group-hover:text-foreground">
														{opt.value}
													</span>
												</div>
						...
						<div className="p-4 flex-1">
							...
							{typoCorrection && query && (
								<div className="mb-3 px-3 py-2 text-xs gap-2 flex items-center rounded-lg border border-border bg-muted text-muted-foreground">
									<span>{t("didYouMean")}</span>
									<button
										type="button"
										className="font-semibold underline underline-offset-2"
										onClick={() => applyChip(typoCorrection)}
									>
										{typoCorrection}
									</button>
							...
							{activeCase === "docs" && query && demo.aiAnswer && (
								<div className="mb-3 px-3 py-2.5 rounded-lg border border-primary/20 bg-primary/5">
									<p className="font-medium mb-1 text-[10px] text-primary">
										AI answer · cited from docs
									</p>
									<p className="text-xs font-light leading-relaxed line-clamp-3 text-foreground">
										{demo.aiAnswer}
									</p>
								</div>
							)}
							...
							{query && results.length > 0 && (
								<p className="font-light mb-2 text-[10px] text-muted-foreground/50">
							...
							{results.length > 0 ? (
								<div className="space-y-0.5">
									{results.map((doc) => (
										<div
											key={doc.id}
											className="group gap-3 px-3 py-2.5 relative flex cursor-default items-start rounded-lg transition-colors hover:bg-muted/50"
										>
											<div className="size-7 rounded text-sm mt-0.5 flex shrink-0 items-center justify-center bg-muted">
												{BADGE_ICONS[doc.badge ?? "Doc"] ?? "📝"}
											</div>
											<div className="min-w-0 flex-1">
												<div className="gap-2 flex flex-wrap items-center">
													<p
														className="text-sm font-light leading-tight text-foreground"
													...
													{doc.badge && doc.badge !== "Best match" && (
														<span className="px-1.5 py-0.5 rounded shrink-0 border border-border bg-muted text-[10px] text-muted-foreground">
															{doc.badge}
														</span>
													)}
													{doc.badge === "Best match" && (
														<span className="px-1.5 py-0.5 rounded shrink-0 border border-primary/20 bg-primary/10 text-[10px] text-primary">
															best match
														</span>
													)}
												</div>
											...
											<button
												type="button"
												className="px-2 py-1 rounded shrink-0 self-center border border-border bg-card text-[10px] whitespace-nowrap text-muted-foreground opacity-0 transition-all group-hover:opacity-100 hover:text-foreground"
												onClick={() =>
													setShowWhy(showWhy === doc.id ? null : doc.id)
												}
											>
												{t("whyResult")}
											</button>
											...
											{showWhy === doc.id && (
												<div className="right-0 mt-1 w-56 shadow-xl p-3 absolute top-full z-30 rounded-lg border border-border bg-card">
													<p className="font-semibold mb-2 tracking-wide text-[10px] text-foreground uppercase">
														Relevance score
													</p>
													{demo.scoreReasons.map((r) => (
														...
														>
															...
															<span className="font-semibold ml-3 text-[10px] text-primary">
																+{r.score}
															</span>
														</div>
													))}
								...
								<div className="py-10 text-center">
									...
									<div className="mt-3 gap-2 flex flex-wrap justify-center">
										{demo.chips.slice(0, 3).map((chip) => (
											<button
												key={chip}
												type="button"
												onClick={() => applyChip(chip)}
												className="px-3 py-1 text-xs rounded-md border border-border text-muted-foreground transition-colors hover:bg-muted"
											>
												{chip}
											</button>
										))}
