...
export function SearchAnalyticsCards({ organizationId, initialTab }: SearchAnalyticsCardsProps) {
	...
	const { data: ctrTrendData } = useQuery(
	...
	// ── CTR trend chart data ──
	const ctrChartData = useMemo(() => {
		if (!ctrTrendData?.trend) return [];
		return ctrTrendData.trend.map((d: { date: string; ctr: number }) => ({
			month: d.date,
			ctr: d.ctr,
		}));
	}, [ctrTrendData]);
	...
	const renderDashboardTab = () => (
		<div className="space-y-6">
			...
			<div className="gap-4 md:grid-cols-2 lg:grid-cols-5 grid grid-cols-2">
				...
				<StatsTile
					title={t("search.analytics.impressions")}
					value={analyticsData?.impressionsCount ?? 0}
					valueFormat="number"
				/>
			...
			{/* Top Clicked Products card */}
			{analyticsData?.topClickedProducts && analyticsData.topClickedProducts.length > 0 && (
				<Card>
					<CardHeader>
	...
	const renderTopQueriesTab = () => (
		<div className="space-y-6">
			<Card>
				<CardHeader>
					<CardTitle>{t("search.analytics.topQueriesTitle")}</CardTitle>
				</CardHeader>
				<CardContent>
					{topQueriesData && topQueriesData.length > 0 ? (
						<Table>
