...
export function EmbeddingModelPanel({ organizationId, slug }: EmbeddingModelPanelProps) {
	...
	if (models) {
		const groups = new Map<string, Array<{ name: string; dimensions: number; maxInputTokens: number }>>();
		for (const m of models) {
			const provider = m.provider;
			if (!groups.has(provider)) groups.set(provider, []);
			groups.get(provider)!.push({ name: m.name, dimensions: m.dimensions, maxInputTokens: m.maxInputTokens });
		}
								...
								<SelectContent>
									{modelGroups.map((group) => (
										<div key={group.provider}>
											<div className="px-2 py-1.5 font-medium text-xs text-muted-foreground uppercase tracking-wider">
												{group.provider}
											</div>
											{group.models.map((m) => (
