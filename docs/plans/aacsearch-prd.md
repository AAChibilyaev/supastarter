...
| 🟡 Partial            | Admin pages (`audit`, `wallet`, `notifications`, `security`) committed as **stubs** (14–21 lines each) — operational purpose to be filled per `06-ui-pages.md §6.2.10`                                            |
...
| ❌ Not started        | `AnalyticsEvent` capture (search/click/zero-results widget events) — widget can emit, but no `POST /api/events` endpoint                                                                                          |
| ❌ Not started        | `searchRouter.recentActivity` — Overview activity feed; only `topQueries` shipped so far                                                                                                                          |
| ❌ Not started        | Stopwords + Presets sub-tabs in `RelevanceTabs.tsx` (Synonyms + Curations shipped already)                                                                                                                        |
...
- v1.x — Recent-activity feed; Stopwords + Presets sub-tabs; Schema Manager; Widget Config UI (DB-unfreeze required for `WidgetConfig`/`AnalyticsEvent`/`AuditLog`)
