...
| 08  | [08-collections-docs-crud.md](08-collections-docs-crud.md) | **Collections & Documents CRUD UX** — fast/spreadsheet-grade table editing. npm packages picked (TanStack Table+Virtual, papaparse, @dnd-kit, use-debounce; reuses RHF+zod+sonner+recharts), route map `[indexSlug]/?tab=…`, schema editor with diff preview, documents table with 3-tier filters + inline edit + bulk actions + drawer detail, import flows (CSV/JSON/connectors), keyboard shortcuts, mobile cards, performance budget, implementation sequence |
