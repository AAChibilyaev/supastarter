...
(function () {
    ...
    var AACSearchWidget = {
        ...
        loadRefinements: function (container, categoryList, tagList) {
            var self = this;
            var url = this.config.apiUrl.replace(/\/+$/, '') +
                '/api/projects/' + encodeURIComponent(this.config.indexSlug) + '/facets';
        ...
        renderRefinementList: function (container, items, name) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                html += '<label>' +
                    '<input type="checkbox" name="' + name + '" value="' + this.escapeHtml(label) + '" /> ' +
        ...
        performSearch: function (container, state, hitsEl, statsEl, paginationEl) {
            ...
            var url = baseUrl + '/api/projects/' +
                encodeURIComponent(this.config.indexSlug) + '/search?' +
            ...
            fetch(url, {
                ...
                .catch(function (err) {
                    state.loading = false;
                    hitsEl.classList.remove('aacsearch-loading');
                    hitsEl.innerHTML = '<div class="aacsearch-error">' +
                        self.escapeHtml('Search error: ' + err.message) + '</div>';
                });
        },
        ...
        renderHits: function (container, hits, query) {
            if (!hits || hits.length === 0) {
                container.innerHTML = '<div class="aacsearch-no-results">' +
                    '<p>No results found' +
        ...
        renderHitCard: function (hit, query) {
            ...
            if (categories.length > 0) {
                breadcrumb = '<div class="aacsearch-hit-breadcrumb">' +
                    this.escapeHtml(categories[0]) + '</div>';
            }
            ...
            var typeBadge = postType ? '<span class="aacsearch-hit-type">' +
                this.escapeHtml(postType) + '</span>' : '';

            return '<article class="aacsearch-hit">' +
                (thumbnail ? '<a href="' + this.escapeHtml(permalink) + '" tabindex="-1">' +
                    '<img class="aacsearch-hit-thumb" src="' + this.escapeHtml(thumbnail) +
                    '" alt="" loading="lazy" /></a>' : '') +
                '<div class="aacsearch-hit-body">' +
                breadcrumb +
                '<h3 class="aacsearch-hit-title"><a href="' + this.escapeHtml(permalink) + '">' +
                title + '</a></h3>' +
                '<p class="aacsearch-hit-excerpt">' + displayExcerpt + '</p>' +
        ...
        initAutocompleteContainer: function (container) {
            ...
            input.addEventListener('keydown', function (e) {
                ...
                switch (e.key) {
                    ...
                    case 'Enter':
                        e.preventDefault();
                        if (self.autocomplete.activeIndex >= 0 && self.autocomplete.items[self.autocomplete.activeIndex]) {
        ...
        renderAutocomplete: function (dropdown, items, query) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                html += '<a href="' + this.escapeHtml(permalink) +
                    '" class="aacsearch-autocomplete-item" data-index="' + i + '">' +
                    (thumbnail ? '<img class="aacsearch-autocomplete-thumb" src="' +
                        this.escapeHtml(thumbnail) + '" alt="" loading="lazy" />' : '') +
                    '<div class="aacsearch-autocomplete-info">' +
                    '<div class="aacsearch-autocomplete-title">' + title + '</div>' +
                    (category ? '<div class="aacsearch-autocomplete-category">' +
                        this.escapeHtml(category) + '</div>' : '') +
                    '</div>' +
                    (postType ? '<span class="aacsearch-autocomplete-badge">' +
                        this.escapeHtml(postType) + '</span>' : '') +
                    '</a>';
            }
        ...
        highlight: function (text, query) {
            ...
            var terms = query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').split(/\s+/).filter(Boolean);
            if (terms.length === 0) {
                return escaped;
            }
