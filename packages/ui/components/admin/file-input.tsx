	...
	return (
		<FormField id={id} name={name} className={cn("w-full", className)} {...rest}>
			...
			<div
				{...getRootProps({
					className: cn(
						"p-6 rounded-lg border-2 border-dashed border-muted text-center transition-colors",
			...
			{children && (
				<div className="previews gap-1 flex flex-col">
					{
						// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const FileInputPreview = (props: FileInputPreviewProps) => {
	...
	return (
		<div className={cn("gap-1 flex flex-row", className)} {...rest}>
			<Button
				type="button"
				variant="ghost"
				size="icon"
				className="h-6 w-6 shadow-sm cursor-pointer rounded-full"
				onClick={onRemove}
