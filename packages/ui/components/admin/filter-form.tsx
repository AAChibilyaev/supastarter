...
const StyledForm = (props: React.FormHTMLAttributes<HTMLFormElement>) => {
	return (
		<form
			{...props}
			className={cn(
				"gap-x-2 gap-y-3 pointer-events-none flex flex-row flex-wrap items-end justify-start",
		...
		>
			...
			{!filterElement.props.alwaysOn && (
				<Button
					...
					variant="ghost"
					size="sm"
					className="hide-filter h-9 w-9 mt-auto cursor-pointer"
					onClick={handleHide}
...
export const FilterButtonMenuItem = React.forwardRef<HTMLDivElement, FilterButtonMenuItemProps>(
	(props, ref) => {
		...
		return (
			<div
				className={cn(
					"new-filter-item px-2 py-1.5 text-sm flex cursor-pointer items-center rounded-sm hover:bg-accent",
